...
class Custom_Post_Type
{
	...
	function register_post_type()
	{
		...
		$labels = array_merge(
			array(
				'name' 					=> sprintf( _x( '%s', 'post type general name', 'custom' ), $this->plural ),
				'singular_name' 		=> sprintf( _x( '%s', 'post type singular title', 'custom' ), $this->title ),
				'menu_name' 			=> sprintf( __( '%s', 'custom' ), $this->plural ),
				'all_items' 			=> sprintf( __( 'All %s', 'custom' ), $this->plural ),
				'add_new' 				=> sprintf( _x( 'Add New', '%s', 'custom' ), $this->title ),
				'add_new_item' 			=> sprintf( __( 'Add New %s', 'custom' ), $this->title ),
				'edit_item' 			=> sprintf( __( 'Edit %s', 'custom' ), $this->title ),
				'new_item' 				=> sprintf( __( 'New %s', 'custom' ), $this->title ),
				'view_item' 			=> sprintf( __( 'View %s', 'custom' ), $this->title ),
				'items_archive'			=> sprintf( __( '%s Archive', 'custom' ), $this->title ),
				'search_items' 			=> sprintf( __( 'Search %s', 'custom' ), $this->plural ),
				'not_found' 			=> sprintf( __( 'No %s found', 'custom' ), $this->plural ),
				'not_found_in_trash' 	=> sprintf( __( 'No %s found in trash', 'custom' ), $this->plural ),
				'parent_item_colon'		=> sprintf( __( '%s Parent', 'custom' ), $this->title ),
			),
			$this->labels
		);
	...
	function add_taxonomy( $name, $args = array(), $labels = array() )
	{
		...
		$taxonomy = new Custom_Taxonomy( $name, $this->name, $args, $labels );
