...
class Custom_Util
{
	...
	{
		return apply_filters( 'custom_beautify', ucwords( str_replace( '_', ' ', $string ) ) );
	}
	...
	{
		return apply_filters( 'custom_uglify', str_replace( '-', '_', sanitize_title( $string ) ) );
	}
	...
	static function pluralize( $string )
	{
        ...
        if ( in_array( strtolower( $string ), $uncountable ) )
        	return apply_filters( 'custom_pluralize', $string );

        // Check for irregular words
        foreach ( $irregular as $noun )
        {
        	if ( strtolower( $string ) == $noun[0] )
            	return apply_filters( 'custom_pluralize', $noun[1] );
        }
        ...
        foreach ( $plural as $pattern )
        {
        	if ( preg_match( $pattern[0], $string ) )
        		return apply_filters( 'custom_pluralize', preg_replace( $pattern[0], $pattern[1], $string ) );
        }
	...
	static function is_reserved_term( $term )
	{
	    ...
	    return new WP_Error( 'reserved_term_used', __( 'Use of a reserved term.', 'custom' ) );
	}
}
